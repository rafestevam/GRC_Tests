...
public class ExplorerPage {
	...
	public boolean verificaRegistrosDuplicados() {
		...
		for (int i = 0; i < length; i++) {
			...
			// Select ckPages2 = getSeletorPaginas(frame, xPathSelect);
			 ...
			 * .until(ExpectedConditions.stalenessOf(element)); List<WebElement>
			 * rows = frame.findElements(By.xpath(xpath));
			 */

			List<WebElement> rows = getTableRows(frame, xpath);

			//Map<String, Integer> mapID = this.getListaID(rows);
	...
	private Select getPageSelector(WebDriver frame, String xPathSelect) {
		/*WebElement ckPages2Elem = new WebDriverWait(frame, 10).until(
				ExpectedConditions.refreshed(ExpectedConditions.presenceOfElementLocated(By.xpath(xPathSelect))));*/
